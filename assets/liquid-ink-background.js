...
(function () {
  ...
  wrapper.style.cssText = "position:fixed;inset:0;width:100%;height:100%;pointer-events:none;overflow:hidden;z-index:0;opacity:0.82;-webkit-mask-image:radial-gradient(ellipse 55% 32% at 50% 16%, rgba(0,0,0,0.35) 0%, rgba(0,0,0,0.75) 55%, rgba(0,0,0,1) 100%);mask-image:radial-gradient(ellipse 55% 32% at 50% 16%, rgba(0,0,0,0.35) 0%, rgba(0,0,0,0.75) 55%, rgba(0,0,0,1) 100%)";
  document.body.insertBefore(wrapper, document.body.firstChild);
  var container = document.createElement("div");
  container.style.width = "100%";
  container.style.height = "100%";
  wrapper.appendChild(container);
  container.dataset.inkVersion = "v18-contrast";
  console.info("[ink] v18-contrast");
  var coarse = window.matchMedia && window.matchMedia("(pointer: coarse)").matches;
  var engine = null;
  try {
    engine = new O(container);
    engine.setConfig({
      transparent: true,
      colorful: false,
      colorPalette: ["#04285B", "#0560E3", "#4ECCEC", "#23E294"],
      hover: false,
      simResolution: 128,
  ...
  var h = setTimeout(function () {
    try { engine.multipleSplats(12); } catch (e) {}
  }, 150);
  var n = setTimeout(function () {
    try { engine.multipleSplats(8); } catch (e) {}
  }, 2200);
  var l = 6000;
  var g = function (v) { return Math.max(-250, Math.min(250, v)); };
  var f = function (x) {
    var c = container.querySelector("canvas");
    return (!c || !c.clientWidth) ? x : x * (c.width / c.clientWidth);
  };
  var T = function (x, y, dx, dy) {
    try {
      engine.splatAtLocation(f(x), y, g(dx / window.innerWidth * l), g(dy / window.innerHeight * l));
    } catch (e) {}
  };
  var y = { x: 0, y: 0, has: false };
  var w = 0;
  var P = function (u) {
    ...
    var R = u.clientX - y.x;
    var E = u.clientY - y.y;
    if (d - w < 24 || R * R + E * E < 4) return;
    y.x = u.clientX;
    y.y = u.clientY;
    w = d;
    T(u.clientX, u.clientY, R, E);
  };
  var B = function (u) {
    T(u.clientX, u.clientY, 0, -18);
    T(u.clientX, u.clientY, 14, 10);
  };
  var I = setInterval(function () {
    if (document.hidden) return;
    var u = window.innerWidth * (0.1 + Math.random() * 0.8);
    var d = window.innerHeight * (0.15 + Math.random() * 0.75);
    T(u, d, (Math.random() - 0.5) * 24, (Math.random() - 0.5) * 24);
    T(u + 30, d + 20, (Math.random() - 0.5) * 18, (Math.random() - 0.5) * 18);
  }, 3500);
  var L = function () {
    try { document.hidden ? engine.stop() : engine.start(); } catch (e) {}
  };
